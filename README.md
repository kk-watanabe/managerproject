...
## 工夫点
本アプリでは、従来分断されていた「案件申請」「進捗管理」「予算管理」を、Webアプリ上で一気通貫に管理できるよう設計しました。

- ロール別画面構成 &rArr; 権限峻別化
- 細部のバリデーション設定や入力ミス防止のUI設計
- 予算超過や遅延発生時のアラート機能 &rArr; 早期対応をプッシュ
- 進捗率バー表示 &rArr; 視認性向上化
- 承認待ち案件数や未読通知をバッジ表示 &rArr; 確認漏れ防止と早期対応を支援

<br>

## 今後の展望
今後の展望は、従来の管理方式で分散していたデータの引継ぎを効率化することです。今回のアプリは分散していたシステムを一元化することですが、新規システムに移行するといっても、過去のデータの移行が重要な問題となると考えられます。今回のアプリではその辺りの事情を考えて設計することができなかったので、CSVのimport/exportなど、データ移行のための便利な機能の実装を行いたいと考えています。<br>
また、100時間という時間の制約があったためあまりUIに凝ることができなかったのですが、ホーム画面のダッシュボード化などよりユーザー目線で見やすく安定感のあるUIの向上を図ろうと思います。
